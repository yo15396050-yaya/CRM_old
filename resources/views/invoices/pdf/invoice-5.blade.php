...
<table class="bg-white" border="0" cellpadding="0" cellspacing="0" width="100%" role="presentation">
    ...
    <tr class="inv-logo-heading">
        ...
        <td align="right" class="f-21 text-black font-weight-700 text-uppercase">
            @lang('app.invoice')</td>
    </tr>
    <tr>
        <td class="f-14 text-grey">
            <p class="mt-3 mb-0">
        ...
        <td align="right">
            <table class="text-black mt-1 f-11 b-collapse rightaligned">
                <tr>
                    <td class="heading-table-left">Facture N°:</td>
                ...
                @endif
                <tr>
                    <td class="heading-table-left">Date :</td>
    ...
    <tr>
        <td class="f-12 text-black">
            ...
            @if (($invoiceSetting->show_client_name == 'yes' || $invoiceSetting->show_client_email == 'yes' || $invoiceSetting->show_client_phone == 'yes' || $invoiceSetting->show_client_company_name == 'yes' || $invoiceSetting->show_client_company_address == 'yes') && $client)
                ...
                <p class="line-height mb-0" style="margin-left:50px;">
                    ...
                    @if ($invoice->clientDetails && $invoice->clientDetails->company_name && invoice_setting()->show_client_company_name == 'yes')
                        Nom de l'entreprise: <b>{{ $invoice->clientDetails->company_name }}</b><br>
                    @endif

                    @if ($invoice->clientDetails && $invoice->clientDetails->office && invoice_setting()->show_client_company_address == 'yes')
                        Téléphone: <b> {!! nl2br($invoice->clientDetails->office) !!}</b><br>
                    @endif                                  

                    @if ($invoice->clientDetails && $invoice->clientDetails->address && invoice_setting()->show_client_company_address == 'yes')
                        Adresse: <b>{!! nl2br($invoice->clientDetails->address) !!}</b>
                    @endif
                    ...
                    @if ($invoiceSetting->show_status)
                        <td align="right">
                            @if ($invoice->status == 'paid')
                                <div style="margin: 0 0 auto auto" class="text-uppercase bg-white {{$invoice->status =='paid' ? $invoice->status:'other'}} rightaligned">
...
<table width="100%" class="f-14 b-collapse">
    ...
    <tr class="main-table-heading text-grey">
        <td width="50%">@lang('app.description')</td>
        @if ($invoiceSetting->hsn_sac_code_show)
            <td align="right">@lang('app.hsnSac')</td>
        @endif
        <td align="right">QTE</td>
        <td align="right">P.U</td>
        <td align="right" width="{{ $invoiceSetting->hsn_sac_code_show ? '30%' : '23%' }}">@lang('modules.invoices.amount')
            (TTC)
        </td>
    </tr>
    <!-- Table Row End -->
    @foreach ($invoice->items->sortBy('field_order') as $item)
        ...
        <!-- Table Row Start -->
            <tr class="f-12 main-table-items text-black">
                ...
                @if ($invoiceSetting->hsn_sac_code_show)
                    <td align="right" width="15%" class="border-bottom-0">
                        {{ $item->hsn_sac_code ?  : '--' }}</td>
                @endif
                <td align="right" width="15%" class="border-bottom-0">{{ $item->quantity }}@if($item->unit)<br><span class="f-11 text-dark-grey">{{ $item->unit->unit_type }}</span>@endif</td>
                <td align="right" width="15%" class="border-bottom-0">{{ currency_format($item->unit_price, $invoice->currency_id, false) }}</td>
                <td align="right" class="border-bottom-0" width="{{ $invoiceSetting->hsn_sac_code_show ? '30%' : '23%' }}">
            ...
            @if ($item->item_summary != '' || $item->invoiceItemImage)
                ...
                <tr>
                    <td colspan="{{ $invoiceSetting->hsn_sac_code_show ? '5' : '4' }}" class="f-13 summary text-black border-bottom-0 description word-break">
    ...
    <!-- Table Row Start -->
    <tr>
        <td class="total-box" align="right" colspan="{{ $invoiceSetting->hsn_sac_code_show ? '4' : '3' }}">
            ...
            @if ($invoice->amountDue())
                <!-- Table Row Start -->
                    <tr align="right" class="balance text-black">
                        <td width="50%" class="balance-left">Montant @lang('modules.invoices.total')
                            TTC</td>
                    </tr>
                    <!-- Table Row End -->
                @endif
...
<table class="bg-white" border="0" cellpadding="0" cellspacing="0" width="100%" role="presentation">
    ...
    @if($invoice->invoicePaymentDetail)
        <table class="description mt-4" style="width:50%; border-collapse: collapse; border: 1px solid #FFFFFF;" cellspacing="0" cellpadding="0">
            <thead>
                <tr style="border-bottom: 1px solid #e7e9eb; color:#555555; background-color:#e7e9eb; font-size: 0.8em;">
            ...
            <tbody>
                <tr style="page-break-inside: avoid;">
                    <td class="description" style="padding: 10px; color:#555555; border: 1px solid #e7e9eb; font-size: 0.7em;">
   ...
   <tr>
        <td style="text-align: left;" width="50%">
            ...
            <div class="f-11 line-height text-grey">
                <p class="text-dark">
                    Par chèque : à l’ordre de « <b>DC-KNOWING CGA</b> »<br>
                    Par virement : <b>CI032 01016 008291570009 63</b><br>
                    Par Mobile Money : Contactez le <b>07 67 13 19 93</b>
                </p>
                <p>Nous vous remercions de votre confiance !</p>
                <p><b>DC-KNOWING THE BESPOKE PARTNER.</b></p> 	        
            </div>
        </td>
        <td style="text-align: right;">
            <h4 class="text-dark-grey">{{-- nl2br($invoiceSetting->invoice_terms) --}} <u>La comptabilité</u></h4>
            @if($invoice->ip_address == 1)
            <img src="{{ $invoiceSetting->authorised_signatory_signature_url }}"
                                             alt="{{ $company->company_name }}" width="50%" id="signature"/>
            @endif
        </td>
    </tr> 
    <!-- Table Row End -->
