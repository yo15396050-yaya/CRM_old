                        ...
                        <div class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-0"
                            ...
                            @endif
                             ...
                             @if (in_array('admin', user_roles()))
                                <hr class="my-1">
                                <a class="dropdown-item send-project-notifications" href="javascript:;" data-project-id="{{ $project->id }}">
...
<script>
    $(document).ready(function() {
        ...
        $('body').on('click', '.send-project-notifications', function() {
            var id = $(this).data('project-id');
            var url = "{{ route('projects.send_notifications', ':id') }}";
            url = url.replace(':id', id);
            ...
            Swal.fire({
                title: "@lang('messages.sweetAlertTitle')",
                text: "Voulez-vous envoyer les notifications par mail et WhatsApp aux intervenants ?",
                icon: 'info',
                showCancelButton: true,
                focusConfirm: false,
                ...
                if (result.isConfirmed) {
                    $.easyAjax({
                        ...
                        success: function(response) {
                            if (response.status == "success") {
                                // Pas besoin de reload si on veut juste rester sur la page
                            }
                        }
                    });
                }
