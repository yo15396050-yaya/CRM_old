...
<style>
    .disabled {
        ...
        border-color: #ced4da; /* Couleur de bordure grise */
        cursor:pointer; /* Curseur pour indiquer que l'élément est désactivé */
    }
</style>
...
@section('content')
    ...
    <div class="content-wrapper">
        ...
        <div class="d-grid d-lg-flex d-md-flex action-bar">
            ...
            <div id="table-actions" class="flex-grow-1 align-items-center">
                ...
                @if(!in_array('apporteurs', user_roles()))
                    <x-forms.link-secondary :link="route('clients.radiation')" class="mr-3 float-left mb-2 mb-lg-0 mb-md-0 d-sm-bloc d-none d-lg-block" icon="times">
                        @lang('Radiation client')
                    </x-forms.link-secondary> 
                @endif   
            </div>
        ...
        <!-- Task Box Start -->
        <div class="d-flex flex-column w-tables rounded mt-3 bg-white table-responsive" id="cacheRad">
...
@push('scripts')
    ...
    <script>
        ...
        $('body').on('click', '.toggle-radiation', function() {
            var userId = $(this).data('clientdetails-id'); 
            var currentValue = $(this).data('current-value');
            var url = "{{ route('clients.apply_toggleRadiation', ':id') }}";
            ...
            $.easyAjax({
                ...
                success: function(response) {
                    if (response.status == "success") {
                        Swal.fire({
                            title: "@lang('messages.sweetAlertTitle')",
                            text: "Vous ne pourrez plus récupérer ce client si votre demande est validée !",
                            icon: 'warning',
                            html: `<div>
                                        <span>Vous ne pourrez plus récupérer ce client si votre demande est validée !</span>
                                        <label for="motif">Motif de la radiation :</label>
                                        <textarea id="motif" name="motif" rows="4" placeholder="Entrez le motif ici..." style="width: 100%;"></textarea>
                                    </div>
                                    `,
                            showCancelButton: true,
                            focusConfirm: false,
                            confirmButtonText: "Oui, radiez-le!",
                            cancelButtonText: "@lang('app.cancel')",
                            customClass: { 
                                confirmButton: 'btn btn-primary mr-3',
                                cancelButton: 'btn btn-secondary'
                            ...
                            if (result.isConfirmed) {
                                ...
                                $.easyAjax({
                                    ...
                                    data: {
                                        '_token': token,
                                        'motif': motif // Inclure le motif dans les données
                                    },
                                    success: function(response) {
                                        if (response.status == "success") {
                                            showTable();
        ...
        $('body').on('click', '.toggle-inactif', function() {
            var userId = $(this).data('inactif-id'); 
            var url = "{{ route('clients.apply_toggleInactif', ':id') }}";
            url = url.replace(':id', userId);
            var token = "{{ csrf_token() }}";
            ...
            $.easyAjax({
                ...
                success: function(response) {
                    if (response.status == "success") {
                        Swal.fire({
                            title: "@lang('messages.sweetAlertTitle')",
                            text: "Vous pourrez récupérer ce client en rendant actif !",
                            icon: 'warning',
                            showCancelButton: true,
                            focusConfirm: false,
                            ...
                            if (result.isConfirmed) {
                                ...
                                var url = "{{ route('clients.toggleInactif', ':id') }}";
                                url = url.replace(':id', userId);
                                $.easyAjax({
                                    type: 'POST',  
                                    url: url,
        ...
        $('body').on('click', '.toggle-actif', function() {
            ...
            $.easyAjax({
                ...
                success: function(response) {
                    if (response.status == "success") {
                        Swal.fire({
                            title: "@lang('messages.sweetAlertTitle')",
                            text: "Récupérer ce client en le rendant actif !",
                            icon: 'warning',
                            showCancelButton: true,
                            focusConfirm: false,
                            confirmButtonText: "Oui !",
