        ...
        <div class="d-grid d-lg-flex d-md-flex action-bar">
            ...
            <div id="table-actions" class="flex-grow-1 align-items-center">
                @if ($addClientPermission == 'all' || $addClientPermission == 'added' || $addClientPermission == 'both')
                    <x-forms.link-primary :link="route('clients.createfiscale')" class="mr-3 float-left mb-2 mb-lg-0 mb-md-0" icon="plus">
                        @lang('Ajouter Situation')
                ...
                success: function(response) {
                    if (response.status == "success") {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                var url = "{{ route('clients.deleteFiscale', ':id') }}";
                                url = url.replace(':id', id);
                                $.easyAjax({
                                    type: 'POST',
                                    url: url,
