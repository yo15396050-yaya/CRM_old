...
class ProjectReminder extends Notification implements ShouldQueue
{
    ...
    public function via($notifiable)
    {
        ...
        $this->message = __('email.projectReminder.subject')."\n".__('email.projectReminder.text').' '.Carbon::now($this->data['global_setting']->timezone)->addDays($this->data['project_setting']->remind_time)->toFormattedDateString();
    ...
    {
        $this->toWhatsapp($notifiable, __($this->smsSetting->slug->translationString(), ['dueDate' => Carbon::now($this->data['global_setting']->timezone)->addDays($this->data['project_setting']->remind_time)->toFormattedDateString()]));
    ...
    public function toMsg91($notifiable)
    {
        if ($this->smsSetting->msg91_flow_id && sms_setting()->msg91_status) {
            return (new \Craftsys\Notifications\Messages\Msg91SMS)
                ...
                ->variable('due_date', Carbon::now($this->data['global_setting']->timezone)->addDays($this->data['project_setting']->remind_time)->toFormattedDateString());
        }
    }
