   ...
   public function store(StoreRequest $request)
{
    ...
    $clients = ClientDetails::where('user_id', $contract->client_id)->first();

    // Si le client existe ET si la date d’adhésion n’est pas encore définie
    if ($clients && empty($clients->skype)) {
        // Définit la date d’adhésion avec la date de création / updated_at
        // updated_at est exactement la date de création ici
        $clients->skype = $contract->updated_at->format('Y-m-d');
        $clients->save();
    }
    ...
    public function updateRenew($id)
    {
        try {
            ...
            if ($clientDetails) {
                // Si la date d'adhésion n'est pas définie, on la définit avec la date actuelle
