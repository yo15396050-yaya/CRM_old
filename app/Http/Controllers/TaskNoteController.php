...
class TaskNoteController extends AccountBaseController
{
    ...
    public function store(StoreTaskNote $request)
    {
        ...
        $note->is_client_visible = $request->has('is_client_visible') && $request->is_client_visible ? 1 : 0;
        $note->deliverables = $request->deliverables;
        $note->status = $note->is_client_visible ? 'approved' : 'approved'; // Par défaut approuvé pour le moment
        $note->save();

        // Déclencher les notifications professionnelles
        $dispatcher = new \App\Services\NotificationDispatcher();
        $dispatcher->dispatchTaskNoteNotification($task, $note);
