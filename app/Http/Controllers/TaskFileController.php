...
class TaskFileController extends AccountBaseController
{
    ...
    {
        \Illuminate\Support\Facades\Log::info("DEBUG TaskFileController::store", [
        ...
        $isAllowed = ($this->addPermission == 'all'
        ...
        if (!$isAllowed) {
            \Illuminate\Support\Facades\Log::warning("DEBUG TaskFileController::store - Permission refusée", [
                'permission' => $this->addPermission,
                'task_added_by' => $task->added_by,
                'user_id' => user()->id
            ]);
            abort_403(false);
        }
        ...
        if ($request->hasFile('file')) {
            \Illuminate\Support\Facades\Log::info("DEBUG - Fichiers trouvés dans la requête: " . count($request->file));
            ...
            foreach ($request->file as $fileData) {
                ...
                try {
                    ...
                    \Illuminate\Support\Facades\Log::info("DEBUG - Fichier uploadé avec succès: " . $filename);
                }
                catch (\Exception $e) {
                    \Illuminate\Support\Facades\Log::error("DEBUG - Erreur Files::uploadLocalOrS3: " . $e->getMessage());
                    throw $e;
                }
