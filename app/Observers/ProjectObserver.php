...
class ProjectObserver
{
    ...
    public function created(Project $project)
    {
        ...
        if (!isRunningInConsoleOrSeeding()) {
            ...
            /* Désactivation de l'ancienne notification de mention (Doublon)
            if ((request()->mention_user_ids) != null && request()->mention_user_ids != '') {
                event(new NewProjectEvent($project, $mentionDescriptionMembers, 'ProjectMention'));
            }
            */

            /* Désactivation des anciennes notifications (Doublons avec NotificationDispatcher)
            // Notification pour les collaborateurs (membres assignés)
            if ($project->allow_collaborator_notification == 'enable') {
                if (!empty(request()->user_id)) {
                    // Si des personnes ont été mentionnées, on les retire de la liste pour ne pas envoyer 2 fois
                    $membersToNotifyIds = request()->user_id;
                    if (!empty($mentionIds)) {
                        $membersToNotifyIds = array_diff($membersToNotifyIds, $mentionIds);
                    }

                    if (!empty($membersToNotifyIds)) {
                        $membersToNotify = User::whereIn('id', $membersToNotifyIds)->get();
                        event(new NewProjectEvent($project, $membersToNotify, 'NewProject'));
                    }
                }
            }
            */
            ...
            if (!empty(request()->client_id) && $project->allow_client_notification == 'enable') {
                event(new NewProjectEvent($project, null, 'NewProjectClient', 'NewProjectClient'));
            }
            */
        }
    }
    ...
    public function updated(Project $project)
    {
        ...
        if (!isRunningInConsoleOrSeeding()) {
            ...
            // Send notification to client
            /* Désactivation de l'ancienne notification de statut (Doublon)
