...
class TasksDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->addColumn(
            ...
            if ($isAdmin) {
                $action .= '<a class="dropdown-item send-task-notifications" href="javascript:;" data-task-id="' . $row->id . '">
                                <i class="fa fa-paper-plane mr-2"></i>
                                ' . __('Informer les bénéficiaires') . '
                            </a>';
            }

            // $action .= '<a href="' . route('messages.index') . '?taskId=' . $row->id . '" class="dropdown-item"><i class="fa fa-comments mr-2"></i>' . __('app.communication') . '</a>';
